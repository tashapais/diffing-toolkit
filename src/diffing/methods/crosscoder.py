...
class CrosscoderDiffingMethod(DiffingMethod):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for layer_idx in self.layers:
            ...
            training_metrics = train_crosscoder_for_layer(
                self.cfg, layer_idx, self.device
            )
        ...
        logger.info(
            f"Crosscoder training completed. Processed {len(results['layers_processed'])} layers."
        )
        return results
