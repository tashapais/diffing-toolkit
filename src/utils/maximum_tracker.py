...
class MaximumTracker:
    ...
    def add_example(
        ...
        if self.tokenizer is not None:
            example_record["text"] = self.tokenizer.decode(
                valid_tokens, skip_special_tokens=False
            )
    ...
    def add_batch_examples(
        ...
        for i in range(batch_size):
            ...
            attention_mask = (
                attention_mask_batch[i] if attention_mask_batch is not None else None
            )
            scores_per_token = (
                scores_per_token_batch[i]
                if scores_per_token_batch is not None
                else None
            )
            additional_data = (
                additional_data_batch[i] if additional_data_batch is not None else None
            )
